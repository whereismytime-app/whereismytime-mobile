...
export function CategoryPieChart({ categoryReports }: CategoryPieChartProps) {
  ...
  const pieData = filteredReports.map((report, index) => {
    ...
    return {
      name: report.category.name.length > 12 
  ...
  return (
    <View className="items-center">
      {/* Chart Title */}
      <Text className="text-lg font-semibold text-gray-900 mb-2">Time Distribution</Text>
      <Text className="text-sm text-gray-600 mb-4">Total: {formatTotalDuration()}</Text>
